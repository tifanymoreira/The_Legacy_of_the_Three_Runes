...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 3. Jogador
    this.player = new Player(this, 200, this.config.height * 0.5);

    // 4. Inimigos (Knights)
    this.knights = this.add.group({ runChildUpdate: true }); 

    const knightBottom = new Knight(this, 600, 450, 'GameScene'); 
    this.knights.add(knightBottom);

    const knightTop1 = new Knight(this, 300, 100, 'GameScene');
    const knightTop2 = new Knight(this, 700, 100, 'GameScene');
    this.knights.add(knightTop1);
    this.knights.add(knightTop2);
    ...
    this.physics.add.collider(this.knights, this.upGround);
    this.physics.add.collider(this.knights, this.stairs);

    // 6. Colecionáveis (Fogo)
    this.createFires(); 
  ...
  // =================================================================
  createBackground() {
    this.add.image(this.config.width * 0.5, this.config.height * 0.5, 'background')
  ...
  createStairs() {
    ...
    for (let i = 0; i < rampWidth; i++) {
      ...
      this.stairs.create(x, y).setSize(1, 1).setVisible(false).setOrigin(0, 1);
    }
  }

  createGround() {
    const groundRect = this.add.rectangle(
      this.config.width / 2, this.config.height * 0.5 + 215, 
  ...
  createUpGround() {
    ...
    const groundRect = this.add.rectangle(
      platformWidth / 1.8, this.config.height * -0.02 + 215, 
      platformWidth, 20
    ).setVisible(false); 
    this.physics.add.existing(groundRect, true);
  ...
  createFires() {
    ...
    const fireLocations = [
      { x: 300, y: 535 }, { x: 500, y: 535 }, { x: 700, y: 535 },
      { x: 100, y: 181 }, { x: 350, y: 181 }, { x: 600, y: 181 }
    ];
  ...
  collectFire(player, fire) {
    ...
    // Condição de Vitória (Vai para Runa 2)
  ...
  createUI() {
    ...
    this.scoreText = this.add.text(
      margin + this.soulIcon.displayWidth + spacing, margin + (soulHeight / 2), '0', 
  ...
  createButton(x, y, text, onClick) {
      ...
      cont.on('pointerover', () => {
        this.buttonSelect.play(); buttonBG.clear().fillStyle(0x333333, 0.9).lineStyle(3, 0xaaaaaa, 1.0)
        .fillRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight, 5)
        .strokeRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight, 5); buttonText.setFill('#ffffff');
      });
      cont.on('pointerout', () => {
        this.buttonSelect.play(); buttonBG.clear().fillStyle(0x1a1a1a, 0.8).lineStyle(3, 0x555555, 1.0)
        .fillRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight, 5)
        .strokeRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight, 5); buttonText.setFill('#E0E0E0');
      });
      cont.on('pointerdown', () => { this.buttonPress.play(); cont.setScale(0.98); });
      cont.on('pointerup', (p) => { 
          this.buttonPress.play(); cont.setScale(1.0); 
