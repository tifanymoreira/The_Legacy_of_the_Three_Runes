...
export default class ThirdRuneScene extends Phaser.Scene {

  constructor(config) {
    super({ key: 'ThirdRuneScene' });
    this.config = config;
  }

  // === RECEBE DADOS DA FASE ANTERIOR ===
  init(data) {
  ...
  create() {
    ...
    // --- PLAYER ---
    this.player = new Player(this, 100, 130); // Posição inicial
    this.player.health = this.initialHealth; 

    // --- INIMIGOS (CAVALHEIROS) ---
    this.knights = this.add.group({ runChildUpdate: true });
    
    const knightPositions = [
        { x: 500, y: 150 }, // Chão (Centro)
        { x: 800, y: 150 }, // Chão (Direita)
        { x: 200, y: 150 }, // Plataforma Alta Esquerda
        { x: 800, y: 150 }, // Plataforma Alta Direita
        { x: 500, y: 150 }  // Plataforma Topo (Guardião da última alma)
    ];

    knightPositions.forEach(pos => {
        const k = new Knight(this, pos.x, pos.y, 'ThirdRuneScene');
        this.knights.add(k);
    });
  ...
  createBackground() {
    this.add.image(this.config.width * 0.5, this.config.height * 0.5, 'third_rune_background')
      .setDisplaySize(this.config.width, this.config.height);
  }

  createGround() {
    const groundRect = this.add.rectangle(
      this.config.width / 2, this.config.height - 200, 
  ...
  createFires() {
    ...
    const fireLocations = [
      { x: 400, y: 150 }, // Canto esquerdo chão
      { x: 600, y: 150 }, // Canto direito chão
      { x: 800, y: 150 }, // Em cima da plat Baixo Esq
    ];
    ...
    this.fires.children.iterate(child => {
      child.body.setSize(40, 40).setOffset(80, 80);
    });
  ...
  collectFire(player, fire) {
    ...
    // SE COLETAR TUDO -> VAI PARA A CUTSCENE DO BOSS
    if (this.score === 16) {
  ...
  startBossTransition() {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('FinalBossCutScene', { 
          score: this.score, 
  ...
  createPauseButton() {
    ...
    pauseContainer.on('pointerdown', () => {
      ...
      this.scene.pause();
      this.scene.launch('PauseScene', { currentSceneKey: 'ThirdRuneScene' });
    });
  }
  ...
  showDeathScreen() {
    ...
    const subtitle = this.add.text(centerX, centerY, 'A Terceira Runa permanece intacta.', {
      fontFamily: 'MedievalSharp, serif', fontSize: '24px', fill: '#ffffff', stroke: '#000', strokeThickness: 3
    }).setOrigin(0.5).setScrollFactor(0).setDepth(2001);

    const btn = this.createButton(centerX, centerY + 100, 'Tentar Novamente', () => {
      bg.destroy(); title.destroy(); subtitle.destroy(); btn.destroy();
      this.cameras.main.fadeIn(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_IN_COMPLETE, () => {
        this.scene.restart();
      });
    });
    btn.setScrollFactor(0).setDepth(2001); 
  }
  ...
  createButton(x, y, text, onClick) {
    ...
    const buttonText = this.add.text(0, 0, text, {
      fontFamily: 'MedievalSharp, serif', fontSize: '32px', fill: '#E0E0E0', stroke: '#000', strokeThickness: 3
    }).setOrigin(0.5);
  ...
  showInstructionMessage() {
    const txt = this.add.text(this.config.width / 2, this.config.height / 2 - 120,
      'A Última Runa. O Guardião está próximo.',
